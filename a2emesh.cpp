 ...
 */
a2emesh::a2emesh() {
	a2emesh::position = (core::vertex*)malloc(sizeof(core::vertex));
	a2emesh::position->x = 0.0f;
	a2emesh::position->y = 0.0f;
	a2emesh::position->z = 0.0f;
}

/*! there is no function currently
 */
a2emesh::~a2emesh() {
	free(a2emesh::position);
}
...
void a2emesh::draw_mesh() {
	for(unsigned int i = 0; i < object_count; i++) {
		...
		for(unsigned int j = 0; j < index_count[i]; j++) {
			...
			glVertex3f(a2emesh::position->x + indices[i][j].v1.x,
				a2emesh::position->y + indices[i][j].v1.y,
				a2emesh::position->z + indices[i][j].v1.z);
			glTexCoord2f(tex_cords[i][j].v2.x, 1.0f - tex_cords[i][j].v2.y);
			glVertex3f(a2emesh::position->x + indices[i][j].v2.x,
				a2emesh::position->y + indices[i][j].v2.y,
				a2emesh::position->z + indices[i][j].v2.z);
			glTexCoord2f(tex_cords[i][j].v3.x, 1.0f - tex_cords[i][j].v3.y);
			glVertex3f(a2emesh::position->x + indices[i][j].v3.x,
				a2emesh::position->y + indices[i][j].v3.y,
				a2emesh::position->z + indices[i][j].v3.z);
		}
		glEnd();
	}
}

void a2emesh::load_textures() {
	SDL_Surface* tex_surface[MAX_OBJS];

	for(unsigned int i = 0; i < texture_count; i++) {
		tex_surface[i] = IMG_LoadPNG_RW(SDL_RWFromFile(tex_names[i], "rb"));
		if(!tex_surface) {
			m.print(msg::MERROR, "a2emesh.cpp", "error loading texture file \"%s\"!", tex_names[i]);
			return;
		}
	}
...
void a2emesh::set_position(float x, float y, float z) {
	a2emesh::position->x = x;
	a2emesh::position->y = y;
	a2emesh::position->z = z;
}
