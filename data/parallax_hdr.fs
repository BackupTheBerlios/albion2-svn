...
void main() {
	...
	vec4 specular_color_rgb = vec4(spec.rgb, 1.0);
	float specular_color_alpha = spec.a * 10.0; // color scale = 10.0
	...
	if(lambertTerm > 0.0) {
		final_color += gl_LightSource[0].diffuse * gl_FrontMaterial.diffuse * lambertTerm * (specular_color_alpha / 7.5) * att;

		vec3 R = reflect(-vview, bump);
		float specular = pow( max(dot(R, vlight), 0.0), gl_FrontMaterial.shininess );

		final_color += gl_LightSource[0].specular * gl_FrontMaterial.specular * specular * specular_color_rgb * specular_color_alpha * att;
	}
