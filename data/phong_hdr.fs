...
void main (void)
{
	...
	vec4 specular_color_rgb = vec4(specular_color.rgb, 1.0);
	float specular_color_alpha = specular_color.a * 10.0; // color scale = 10.0
	...
	if(lambertTerm > 0.0)
	{
		...
		final_color += gl_LightSource[0].specular * gl_FrontMaterial.specular * specular * specular_color_alpha * specular_color_rgb * att;
	}
